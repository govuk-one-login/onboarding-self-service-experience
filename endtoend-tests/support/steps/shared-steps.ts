...
import {
    ...
    getEmail,
    extractSecurityCodeFromEmail,
    postSecurityCodeBackToCognito
...
Then("wait for email {}", {timeout: 3 * 5000}, async function (emailSubject) {
    await getEmail(emailSubject);
});

Then("extract the security code from email", async function (): Promise<void> {
    await extractSecurityCodeFromEmail();
});

Then("register the code with Cognito", async function (): Promise<void> {
    await postSecurityCodeBackToCognito(this.page);
});

Then("pause for registration to complete", {timeout: 1 * 5000}, async function () {
    console.log("Comment to allow Lint Tests to pass");
});
