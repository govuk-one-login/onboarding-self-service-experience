...
import {onlyNumbers} from "./mobileValidator"
import {ukNumberCheck} from "./mobileValidator"
import {beginsWith070} from "./mobileValidator"
import {lengthCheck} from "./mobileValidator"
import {convert07Numbers} from "./mobileValidator"
import {removeSpaceAndDash} from "./mobileValidator"
...
describe('Checking that only numbers starting with +447[1-9] and 07[1-9] are accepted', () => {
   it('An invalid +48 number is rejected.', async function () {
       assert.equal(await ukNumberCheck(INVALID_NUMBER_STARTING_WITH_PLUS_48), false, `${INVALID_NUMBER_STARTING_WITH_PLUS_48} should not be allowed as it does not begin with +447`);
   })
 
   it('A valid +44 number is accepted.', async function () {
       assert.equal(await ukNumberCheck(VALID_NUMBER_STARTING_WITH_PLUS_44), true, `${VALID_NUMBER_STARTING_WITH_PLUS_44} should be allowed as it begins with +447`);
   })
 
   it('An invalid 08 number is rejected.', async function () {
       assert.equal(await ukNumberCheck(NUMBER_BEGINNING_WITH_08), false, `${NUMBER_BEGINNING_WITH_08} should not be allowed as it does not begin with +447`);
   })
 
   it('An valid 07 number is accepted.', async function () {
       assert.equal(await ukNumberCheck(VALID_NUMBER_STARTING_WITH_0), true, `${VALID_NUMBER_STARTING_WITH_0} should be allowed as it begins with 07`);
   })
...
describe('Checking that numbers starting with +4470 and 070 are recognised', () => {
   ...
   it('Checking that numbers which begin with 070 or +4470 are recognised . ', async function () {
       assert.equal(await beginsWith070(NUMBER_BEGINNING_WITH_070), true, `${NUMBER_BEGINNING_WITH_070} should be recognised as a number beginning with 070`);
       assert.equal(await beginsWith070(NUMBER_BEGINNING_WITH_PLUS_4470), true, `${NUMBER_BEGINNING_WITH_PLUS_4470}  should be recognised as a number beginning with +4470`);
   })
 
   it('Checking that numbers which begin with 07 or +447 are rejected. ', async function () {
       assert.equal(await beginsWith070(VALID_NUMBER_STARTING_WITH_0), false, `${VALID_NUMBER_STARTING_WITH_0} should be rejected`); // not a 070 number
       assert.equal(await beginsWith070(VALID_NUMBER_STARTING_WITH_PLUS_44), false, `${VALID_NUMBER_STARTING_WITH_PLUS_44} should be rejected`); // not a 070 number
   })
