import {obscureNumber, convertToCountryPrefixFormat, validateNumber} from "../../src/lib/mobile-number";
...
describe("Validate and format mobile phone numbers", () => {
    describe("Reject invalid numbers", () => {
        it("Reject a blank value", () => {
            expect(validateNumber("")).toEqual({isValid: false, errorMessage: "Enter a mobile phone number"});
        });
        ...
        it("Reject a number including an illegal symbol", () => {
            expect(validateNumber(`${INTERNATIONAL_PREFIX}${NETWORK_CODE}_${FIRST_THREE_DIGITS}_${LAST_THREE_DIGITS}`)).toEqual({
                isValid: false,
        ...
        describe.each(invalidNumbers)("Reject incorrectly formatted numbers", number => {
            it(number, () => {
                expect(validateNumber(number).isValid).toEqual(false);
            });
        });
    });
