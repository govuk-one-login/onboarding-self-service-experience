...
describe("Checking that the user has entered a valid OTP", () => {
    ...
    it("an OTP with less than 6 digits is not accepted. ", async function () {
        assert.equal(
            sixDigits(OTP_LESS_THAN_6_DIGITS),
            false,
            `Expected ${OTP_LESS_THAN_6_DIGITS} not to be allowed because it does not have 6 digits`
        );
    });

    it("a password with empty value is not accepted. ", async function () {
        assert.equal(
            sixDigits(OTP_WITH_EMPTY_VALUE),
            false,
            `Expected ${OTP_WITH_EMPTY_VALUE} not to be allowed because it has empty value`
        );
    });
