...
ShowPassword.prototype.init = function () {
    ...
    const disableFormSubmitCheck = this.$module.getAttribute("data-disable-form-submit-check");
...
ShowPassword.prototype.togglePassword = function (event) {
    ...
    this.showHide.setAttribute(
        "aria-label",
        this.showHide.getAttribute("aria-label") === this.showPasswordFullText ? this.hidePasswordFullText : this.showPasswordFullText
    );
...
ShowPassword.prototype.revertToPasswordOnFormSubmit = function () {
    this.showHide.innerHTML = this.showPasswordText;
    this.showHide.setAttribute("aria-label", this.showPasswordFullText);
    this.statusText.innerHTML = this.hiddenPassword;
...
ShowPassword.prototype.moveDataAttributesToButton = function () {
    const attrs = this.input.attributes;
...
for (const passwordField of window.document.querySelectorAll(".gem-c-show-password")) {
    new ShowPassword(passwordField).init();
}
